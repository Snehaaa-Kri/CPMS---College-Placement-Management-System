...
function LandAbout() {
  const roles = [
    ...
    {
      title: "CDC(Career Development Cell)",
      image: TPO,
      description:
        "CDCs manage student's data, job postings, application reviews, and generate insightful reports for placement tracking.",
    },
    {
      title: "Management",
      image: Management,
  ...
  return (
    <div
      id="about"
      className="bg-gradient-to-tr from-[#F6B56F] via-[#C0690C] to-[#C0690C] py-10 scroll-mt-24"
    >
      <div className="text-center mb-12">
        ...
        <p className="text-md md:text-lg max-w-3xl mx-auto text-gray-700 px-3">
          CPMS simplifies and manages the entire campus placement process by connecting students, TPOs, and recruiters in one secure and easy-to-use platform.
        ...
        {roles.map((role, idx) => (
          ...
          >
            <img
              src={role.image}
              alt={role.title}
              className="w-48 h-48 object-cover rounded-full border-4 border-[#C0690C] shadow-md"
            />
            <h3 className="text-xl md:text-2xl font-semibold mt-4 mb-2 text-[#C0690C] text-center">{role.title}</h3>
